...
We will use the complete RNA-seq raw sequences from Ariel et al. \cite{Ariel2021}. However, rather than using the 72 samples presented in the article, we will focus on 24 representative samples of this study:

The \textbf{samples.list} file located in the /data directory contains the SRA identifiers of these 24 samples:
\begin{verbatim}
SRS2153774
SRS2153779
SRS2153781
...
Simply download the 24 samples with :

\begin{verbatim}
prefetch --option-file samples.list
\end{verbatim}

To extract fq files from SRA files, the fasterq-dump command is required:

\begin{verbatim}
fasterq-dump --split-files SRR5487396.sra
\end{verbatim}

In practice, you will want to do it for all folders. Navigate to the /data folder and use the following qsub command to lauch the faster-qdump commands sequentially:

\begin{verbatim}
qsub faster-qdump.sh
\end{verbatim}

where \textbf{faster-qdump.sh} is a simple bash script :
\begin{verbatim}
	#! /bin/bash
	#$ -S /bin/bash
	#$ -cwd
